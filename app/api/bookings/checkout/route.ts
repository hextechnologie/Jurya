...
export async function POST(request: NextRequest) {
  try {
    ...
    const candidateName = String(user.user_metadata?.full_name || '').trim() || 'Candidat'
    const coachDisplayName = String(coachName || '').trim() || 'Membre de jury'
    ...
    const baseBookingPayload = {
      ...
      google_calendar_url: `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(`Session avec ${coachDisplayName}`)}&details=${encodeURIComponent(notes || 'Session de coaching')}`,
    }
    ...
    await supabase.from('credit_transactions').insert({
      ...
      description: `Réservation d'une session de ${durationMinutes} min avec ${coachDisplayName}`,
    ...
    await supabase.from('notifications').insert({
      ...
      message: `${candidateName} a réservé une session de ${durationMinutes} minutes pour le ${scheduledAt}.`,
      type: 'booking',
      read: false,
    })
    ...
    if (process.env.RESEND_API_KEY) {
      ...
      await resend.emails.send({
        ...
        subject: `Réservation confirmée avec ${coachName}`,
        html: `<p>Votre session de ${durationMinutes} minutes avec ${coachName} est confirmée pour le ${scheduledAt}.</p><p>Coût : ${creditsCost} crédits</p><p>Notes : ${notes || 'Aucune'}</p><p>Vous pouvez annuler avec remboursement intégral jusqu'à 48h avant la session.</p>`,
      })
    }

    return NextResponse.json({ bookingId, calendarUrl, message: 'Réservation créée avec succès' })
  } catch (error: any) {
