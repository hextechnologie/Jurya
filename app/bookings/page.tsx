    ...
    <div className="min-h-screen bg-background text-white px-6 py-8">
      <div className="mx-auto max-w-5xl">
        ...
        {loading ? (
          ...
          <Card>
            <p className="text-gray-400 text-center">Aucune réservation {filter === 'upcoming' ? 'prévue' : filter === 'past' ? 'passée' : 'annulée'}</p>
          </Card>
        ) : (
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {userRole === 'coach' ? 'Candidat' : 'Membre de jury'}
                      </p>
                    </div>
                  ...
                  {booking.status === 'cancelled' && booking.cancelled_at && (
                    <div className="mb-4 p-3 rounded-lg bg-red-500/10 border border-red-500/40">
                      <p className="text-xs text-red-300">
                        Annulée le {new Date(booking.cancelled_at).toLocaleDateString('fr-FR')} par {booking.cancelled_by}
                      </p>
                    </div>
                  )}
